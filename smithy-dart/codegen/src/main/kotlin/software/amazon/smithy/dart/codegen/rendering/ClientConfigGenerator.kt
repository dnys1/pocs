...
import software.amazon.smithy.dart.codegen.core.withBlock
import software.amazon.smithy.dart.codegen.model.hasIdempotentTokenMember
...
class ClientConfigGenerator(
    ...
    private fun registerDefaultProps() {
        props.add(DartClientRuntimeConfigProperty.SdkLogMode)
        if (ctx.protocolGenerator?.applicationProtocol?.isHttpProtocol == true) {
            props.add(DartClientRuntimeConfigProperty.HttpClientEngine)
        }
        if (ctx.shape != null && ctx.shape.hasIdempotentTokenMember(ctx.model)) {
            props.add(DartClientRuntimeConfigProperty.IdempotencyTokenProvider)
        }
    }
