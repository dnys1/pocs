...
import software.amazon.smithy.dart.codegen.rendering.serde.formatInstant
...
class HttpStringValuesMapSerializer(
    ...
    fun render(
        ...
        bindings.sortedBy(HttpBindingDescriptor::memberName).forEach {
            ...
            when (memberTarget) {
                ...
                is BlobShape -> {
                    writer.addImport("encodeBase64String", DartDependency.UTILS)
                    writer.write(
    ...
    private fun renderCollectionShape(binding: HttpBindingDescriptor, memberTarget: CollectionShape, writer: DartWriter) {
    ...
    private fun renderStringShape(binding: HttpBindingDescriptor, memberTarget: StringShape, writer: DartWriter) {
        val memberName = symbolProvider.toMemberName(binding.member)
