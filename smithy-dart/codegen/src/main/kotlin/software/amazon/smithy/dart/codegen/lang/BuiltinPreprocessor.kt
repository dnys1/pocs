...
import software.amazon.smithy.dart.codegen.core.clientName
 ...
 */
class BuiltinPreprocessor : DartIntegration {
    ...
    private fun getRenamed(model: Model, settings: DartSettings): Map<ShapeId, ShapeId> {
        val serviceNamespace = settings.getService(model).id.namespace
        val kotlinBuiltins = DartTypes.All.map { it.name }.toSet()
